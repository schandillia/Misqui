...
export default function ColorSwitcher({
  ...
  return (
    <div
      className={cn(
        "flex items-center",
        inSidebar ? "flex-col gap-1" : "flex-row gap-2"
      )}
    >
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            size="icon"
            className={cn(
              inSidebar ? "size-7" : "size-8",
              `rounded-full border border-border transition-all hover:scale-110 cursor-pointer
              ...
              after:from-white/40 after:to-transparent after:rounded-full`,
              `ring-2 ring-brand-300/50 hover:ring-3 hover:ring-brand-400/70
              dark:ring-brand-500/50 dark:hover:ring-brand-600/70`,
              brandColor
            )}
        ...
        <DropdownMenuContent
          align="end"
          className={cn(
            `w-48 sm:w-fit rounded-3xl border border-border bg-neutral-100/80
          ...
          <div className="grid grid-cols-4 gap-2 sm:gap-4">
            {brandColors.map((color) => (
              <DropdownMenuItem
                ...
                className={cn(
                  `group flex items-center justify-center sm:justify-start gap-2 p-1 sm:p-2
                  rounded-3xl hover:bg-brand-100/50`,
                  color.className
                )}
              >
                <div
                  className={cn(
                    "size-6 rounded-full transition-all flex-shrink-0",
      ...
      <span
        className={cn(
          ...
          inSidebar ? "text-center" : "text-left",
          "transition-colors duration-300",
          "animate-fade-in" // Subtle fade-in animation
        )}
      >
        {currentColorLabel}
      </span>
